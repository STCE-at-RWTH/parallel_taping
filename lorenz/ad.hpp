...
#define AD_VERSION_SHORT "AD v1.5"
...
namespace ad {
  ...
  template <typename T>
  struct ad_allow_explicit_type_cast_to { const static bool value = false; };
}
    ...
    namespace operations {
        ...
        template<class AD_TAPE_REAL>struct ad_asin {
            template<class T>static inline const AD_TAPE_REAL eval(const T &arg) {
                ...
                (void)_value;
                return (static_cast<AD_TAPE_REAL>(1.0) / sqrt(static_cast<AD_TAPE_REAL>(1.0) - x._value()*x._value()));
            }
        };
        using std::acos;
        ...
        template<class AD_TAPE_REAL>struct ad_atan {
            template<class T>static inline const AD_TAPE_REAL eval(const T &arg) {
                ...
                (void)_value;
                return (static_cast<AD_TAPE_REAL>(1.0) / (static_cast<AD_TAPE_REAL>(1.0) + x._value()*x._value()));
            }
        };
        using std::tanh;
        ...
        template<class AD_TAPE_REAL>struct ad_sqrt {
            template<class T>static inline const AD_TAPE_REAL eval(const T &arg) {
                ...
                (void)_value;
                return (static_cast<AD_TAPE_REAL>(1.0)/(static_cast<AD_TAPE_REAL>(2.0)*sqrt(x._value() + (x._value() > 0 ? static_cast<AD_TAPE_REAL>(0.0) : static_cast<AD_TAPE_REAL>(1e-8)))));
            }
        };
        using std::log;
        ...
        template<class AD_TAPE_REAL>struct ad_erf {
            template<class T>static inline const AD_TAPE_REAL eval(const T &arg) {
                ...
                (void)_value;
                return (static_cast<AD_TAPE_REAL>(2.0) / static_cast<AD_TAPE_REAL>(sqrt(3.14159265358979323846264338327950288)) * exp(-x._value() * x._value()));
            }
        };
        using ::erfc;
        ...
        template<class AD_TAPE_REAL>struct ad_acosh {
            template<class T>static inline const AD_TAPE_REAL eval(const T &arg) {
                ...
                (void)_value;
                return (static_cast<AD_TAPE_REAL>(1.0) / sqrt((x._value()*x._value()) - static_cast<AD_TAPE_REAL>(1.0)));
            }
        };
        using ::expm1;
...
namespace ad {
    namespace internal {
        template<class AD_TAPE_REAL, class AD_ARG, class AD_OPERATION> struct unary_intermediate {
            ...
            template <typename TYPE, typename ALLOW_CAST = typename std::enable_if<ad_allow_explicit_type_cast_to<TYPE>::value>::type>
...
namespace ad {
    namespace internal {
        ...
        struct ts_data {
            ...
            inline void clear() {
                tlm = static_cast<AD_TAPE_REAL>(0.0);
            }
...
namespace ad {
    namespace internal {
        ...
        struct blob_tape {
            ...
            public:
                blob_tape() : _topOfStack(0), _progvarcounter(0), _adjoints(0), _stack(0), _stack_size(0), _adjoint_size(0), _isdead(false), _isactive(false), _vecidx(0), _initialized(0) {
                }
            public:
                ...
                inline TAPE_ENTRY *_get_insert_ptr(const int num_entries2fill, AD_TAPE_INT &new_tape_index) {
                    ...
                    if(_progvarcounter > _adjoint_size){
                        throw ad::exception::create<std::runtime_error>("Size of adjoint vector exceeded. Please allocate a bigger tape.");
                    }
                    if(_topOfStack > _stack + _stack_size){
                        throw ad::exception::create<std::runtime_error>("Size of tape partial stack exceeded. Please allocate a bigger tape.");
                    }
                    return ret;
                }
            ...
            public:
                ...
                inline size_t get_tape_memory_size() const {
                    return (get_position()._progvarcounter() * sizeof(AD_TAPE_REAL) + get_position()._stackcounter() * sizeof(TAPE_ENTRY)) / 1024.0 / 1024.0;
                }
                // return allocated memory size in MB
                inline double get_allocated_tape_memory_size() const {
                    return ((_adjoint_size) * sizeof(AD_TAPE_REAL) + (_stack_size) * sizeof(TAPE_ENTRY)) / 1024.0 / 1024.0;
                }
                ...
                struct single_tape_data : handler_base {
                    ...
                    public:
                        ...
                        inline AD_ADJOINT_REAL &_adjoint() const {
                            AD_TAPE_INT& init = global_tape->_initialized;
                            if(init < _tape_index_){
                                while((init++) < _tape_index_){
                                   global_tape->_adjoint(init) = 0; 
                                }
                            }
...
namespace ad {
    ...
    template <typename T> typename adjoint<T>::tape_t *adjoint<T>::global_tape;
}
namespace ad {
    ...
    struct mode<ad::internal::active_type<AD_TAPE_REAL, typename ad::tangent<AD_TAPE_REAL>::_data> > : public ad::tangent<AD_TAPE_REAL> {};
    template <typename AD_TAPE_REAL>
    struct mode<ad::internal::active_type<AD_TAPE_REAL, typename ad::adjoint<AD_TAPE_REAL>::_data> > : public ad::adjoint<AD_TAPE_REAL> {};
